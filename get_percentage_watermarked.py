...
train = True
data_path = f'datasets/watermark/dataset_{percentage}{"" if train else "_test"}.pickle'
...
with torch.no_grad():
    for i, (x, yx, z, yz) in enumerate(tqdm(train_loader)):
        ...
        to_replace = torch.rand((xh.shape[0], 1, 1, 1)) < percentage
        ...
        xh = torch.where(to_replace, xh, x)  # If true, keep watermark xh. Otherwise, use x
